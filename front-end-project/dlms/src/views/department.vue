  ...
  <keep-alive>
    <div>
      <template style="width: 100%">
        <el-button style="float: right" type="primary" icon="el-icon-document-add" size="small" @click="addCollege"
                   plain>添加
        </el-button>
      </template>
      <el-table
...
export default {
  ...
  created() {
    let vue = this;
    let collegestate = sessionStorage.getItem('departmentstate');
  ...
  methods: {
    ...
    handleComplete(index, row) {
      ...
      if (this.tableData[index].isSet == true) {
        ...
        this.$axios.post('/baseinfo/setDepartment', data).then(res => {
          if (res.data.code == 200) {
    ...
    addCollege() {
      ...
      this.$prompt('请输入部门名称', '提示', {
        ...
        vue.$axios.post('/baseinfo/setDepartment', data).then(res => {
          if (res.data.code == 200) {
            // vue.$message.success(res.data.msg);
            vue.$notify.success(res.data.msg);
